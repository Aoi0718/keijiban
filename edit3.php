...
___EOF___;
$sql = 'SELECT * FROM movie';       // テーブルの読み出し(表示)
...
$body = str_replace('<!--RECORD_LIST-->', $rlist, $FORM_BODY ); // 作成したフォームを差し込む
    ...
    if( $_POST["mode"] == "select" ) {
        ...
        $sql = "SELECT * FROM movie WHERE ID={$target}";    // 対象のレコードを取り出す
...
if ( $record['media'] == "DVD" ){
    ...
    $body = str_replace('<!--BR-->', "checked", $body );
        ...
        $fprice = $_POST['price'];
        // SQLの実行1
        $sql ="UPDATE movie SET name='{$fname}', media='{$fmedia}',price={$fprice} WHERE ID={$fid}";
